...
// License: Apache-2.0
// Lato changes: nest under script; WorkflowOutcome → ScriptOutcome; crate paths via super; reject empty agent("") prompt.
...
use super::host::{AgentOpts, HostError, WorkflowHostRequest};
use super::journal::{HOST_ERROR_KEY, Journal, JournalError, request_hash};
use super::run::{PauseKind, ScriptOutcome};
use super::{MAX_HOST_CALLS, MAX_PARALLEL};
...
enum PendingAgent {
    ...
    Live {
        seq: u64,
        hash: String,
        reply_rx: oneshot::Receiver<Result<super::host::AgentResult, HostError>>,
    },
}
...
fn outcome_from_error(err: EvalAltResult) -> ScriptOutcome {
    if let Some(token) = find_control_token(&err) {
        return match token {
            ControlToken::Complete(result) => ScriptOutcome::Completed { result },
            ControlToken::Pause(kind, message) => ScriptOutcome::Paused { kind, message },
            ControlToken::Budget(message) => ScriptOutcome::BudgetExceeded { message },
            ControlToken::Cancelled => ScriptOutcome::Cancelled,
            ControlToken::Fatal(error) => ScriptOutcome::Failed { error },
        };
    }
    ScriptOutcome::Failed {
        error: super::with_rhai_hint(err.to_string()),
    }
}
...
fn register_host_fns(engine: &mut rhai::Engine, ctx: &Rc<RefCell<Ctx>>) {
    ...
    engine.register_fn("fingerprint", |text: &str| -> String {
        super::journal::request_hash("fingerprint", &serde_json::Value::String(text.to_string()))
    });
...
mod tests {
    use super::*;
    use crate::{AgentResult, BudgetState};
    ...
    fn cancellation_wins_over_pure_loop() {
        ...
        p.cancel = cancel;
        assert!(matches!(run_workflow(p), ScriptOutcome::Cancelled));
    }
    ...
    fn parallel_budget_exceeded_leaves_panel_unjournaled_for_raised_cap_resume() {
        ...
        let outcome = run_workflow(params(script, Journal::new(Some(journal_path.clone())), tx));
        drop(host);
        assert!(matches!(outcome, ScriptOutcome::BudgetExceeded { .. }));
        ...
        drop(host);
        assert!(matches!(replay, ScriptOutcome::Completed { .. }));
    ...
    fn cancelled_live_agent_releases_budget_so_resume_does_not_double_charge() {
...

        {
            ...
            drop(host);
            assert!(matches!(outcome, ScriptOutcome::Cancelled));
            assert_eq!(
    ...
    fn cancelled_parallel_releases_budget_so_resume_does_not_double_charge() {
...

        {
            ...
            drop(host);
            assert!(matches!(outcome, ScriptOutcome::Completed { .. }));
            assert_eq!(
                agents_used.load(Ordering::SeqCst),
                2,
    ...
    fn journal_replay_skips_host_calls() {
        ...
        drop(host);
        let ScriptOutcome::Completed { result: first } = first else {
        ...
        drop(host);
        let ScriptOutcome::Completed { result: second } = second else {
    ...
    fn journal_divergence_fails_loudly() {
        ...
        drop(host);
        assert!(matches!(first, ScriptOutcome::Completed { .. }));
