// Derived from: Grok Build@bb7f39d5858cbf5e00de639367f59debbdcb0138:crates/codegen/xai-workflow/src/validate.rs
// License: Apache-2.0
// Lato changes: nest under script; ScriptOutcome; DEFAULT_AGENT_BUDGET u32→u64; canned host keeps agent_budget.

use super::engine::{WorkflowRunParams, run_workflow};
use super::host::{AgentResult, BudgetState, WorkflowHostRequest};
use super::meta::MetaError;
use super::{HostError, Journal, ScriptOutcome, extract_meta};
...
pub fn validate_script(
    ...
    validate_script_with_agent_budget(script, args, u64::from(crate::DEFAULT_AGENT_BUDGET))
}
...
pub fn validate_script_with_cancel(
    ...
    validate_script_with_agent_budget_and_cancel(
        script,
        args,
        u64::from(crate::DEFAULT_AGENT_BUDGET),
        cancel,
    )
}
...
fn validate_script_with_agent_budget_and_cancel(
    ...
    let (outcome_ok, outcome_summary) = match &outcome {
        ...
        ScriptOutcome::Failed { error } => (false, format!("failed: {error}")),
