...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum ActiveMessageRetirementProof {
    Revoked,
    Claimed,
    Committed,
}
...
impl ActiveMessageAdmissionLease {
    ...
    pub(crate) fn retire(&self) -> ActiveMessageRetirementProof {
        loop {
            match self.state.load(Ordering::Acquire) {
                value if value == ActiveMessageLeaseState::Open as u8 => {
                    if self
                        .state
                        .compare_exchange(
                            ActiveMessageLeaseState::Open as u8,
                        ...
                        .is_ok()
                    {
                        return ActiveMessageRetirementProof::Revoked;
                    }
                }
                value if value == ActiveMessageLeaseState::Claimed as u8 => {
                    return ActiveMessageRetirementProof::Claimed;
                }
                value if value == ActiveMessageLeaseState::Committed as u8 => {
                    return ActiveMessageRetirementProof::Committed;
                }
                value if value == ActiveMessageLeaseState::Revoked as u8 => {
                    return ActiveMessageRetirementProof::Revoked;
                }
                _ => unreachable!("active-message lease state is valid"),
            }
        }
    }
}
