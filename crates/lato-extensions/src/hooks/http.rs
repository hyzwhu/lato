...
use super::{HookEventEnvelope, HookRunContext, HookRunError, HookSpec, MAX_PAYLOAD_BYTES, MAX_RUNNER_OUTPUT_BYTES, RawHookRun};
...
pub async fn validate_hook_url(url: &Url, resolver: &dyn HookDnsResolver) -> Result<(), HookRunError> {
    ...
    let addresses = resolver.resolve(host, port).await.map_err(|_| HookRunError::UnsafeUrl)?;
...
pub async fn run_http_hook(
    ...
    if payload.len() > MAX_PAYLOAD_BYTES { return Err(HookRunError::PayloadTooLarge); }
    let configured = spec.url.as_deref().ok_or(HookRunError::InvalidConfiguration)?;
    ...
    let operation = async {
        ...
        let response = client.post(url).header(reqwest::header::CONTENT_TYPE, "application/json").body(payload).send().await.map_err(|_| HookRunError::Http)?;
        let status = response.status();
        if status.is_redirection() || !status.is_success() { return Err(HookRunError::Http); }
        ...
        Ok(RawHookRun { stdout: String::from_utf8_lossy(&body).into_owned(), stderr_preview: String::new(), exit_code: Some(0), elapsed: started.elapsed(), truncated: false })
    };
    tokio::select! {
        biased;
...
fn expand_url(value: &str, spec: &HookSpec, context: &HookRunContext<'_>) -> String {
    ...
    let identities = [
        ...
        ("LATO_WORKSPACE_ROOT", context.workspace_root.to_str().unwrap_or("")),
        ("CLAUDE_PROJECT_DIR", context.workspace_root.to_str().unwrap_or("")),
    ];
    for (key, value) in spec.extra_env.iter().map(|(key, value)| (key.as_str(), value.as_str())).chain(identities) {
...
fn blocked(ip: IpAddr) -> bool {
    match ip {
        ...
        IpAddr::V6(ip) => {
            if ip.is_loopback() { return false; }
            if let Some(mapped) = ip.to_ipv4_mapped() { return blocked(IpAddr::V4(mapped)); }
            ip.is_unspecified() || ip.is_multicast() || ip.is_unique_local() || ip.is_unicast_link_local()
        }
    }
}
