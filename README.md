...
Runtime risk note (7C2 semantics, frozen by the pinned contract): a remote AgentField execution keeps running after the Lato process exits — closing a session never implicitly cancels remote runs, and cancellation is always explicit. The pinned `v0.1.138` async-start endpoint does not support idempotency keys, so when a `start` outcome is ambiguous (no execution ID received), Lato will never auto-retry, auto-query, or auto-reconcile; the run is recorded as a permanent local `outcome_unknown` terminal state and only manual reconciliation in the AgentField control plane is offered. Never issue a second `start` for the same work unless a duplicate execution is acceptable.

Migration and rollback: set `enabled: false` (or delete the `agentfield` section) to deactivate the adapter; unconfigured/disabled states make no network probes, resolve no credentials, and leave all existing task/workflow behavior untouched (tool catalog and goldens unchanged). In Phase 7C1 the rollback boundary is complete: the adapter registers no tool and writes no journal events, so rolling back the binary is always safe. Once Phase 7C3 introduces versioned AgentField journal events, that changes — sessions that contain 7C events cannot be read by older binaries, and binary downgrade for such sessions will be blocked by an explicit reader-version gate (per the frozen v1.2 spec §10). Run `lato doctor` (offline) after config changes and `lato doctor --live` before first use.
