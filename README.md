...
```text
/help  /new  /clear  /compact  /sessions  /rename  /delete  /model  /login  /doctor  /search  /lang  /approve  /status  /permissions  /skills  /skill  /files  /exit
```

During generation, reasoning text returned by the provider appears in a live card with an animated indicator, elapsed time and the latest lines. Each segment folds when the model starts answering or calls a tool; F2 expands/collapses reasoning records. Providers without a reasoning stream show actual waiting, tool execution and answer states instead. The bottom bar always shows the latest input-token estimate, known context window and utilization, model, activity and elapsed time. Unknown context values are shown as `—`; estimates remain visible while the model is generating.

The default layout gives the conversation the full width. Tab opens the tool panel, Shift-Tab opens sessions, and Esc returns to chat. Type `/` to search commands and installed plugin skills by name or description; Ctrl-K opens the same searchable command palette. Up/Down selects a candidate, Tab completes it, and Esc dismisses the menu while preserving the draft. Enter accepts a candidate first; an exact built-in command executes on Enter. Unknown commands keep the draft for correction.

Type `@` (or `/files`) to search workspace files. Selecting a file inserts `@path`, or `@"path with spaces"`; sending includes its UTF-8 contents and source in the model request. File search respects ignore rules and excludes generated directories. References remain inside the workspace and allow at most 64 KiB per file and 256 KiB total. Missing, binary or oversized files show an error and keep your draft. `/files` refreshes the index. An `@word` at a word boundary is an explicit file reference; email addresses are ordinary text.

`/skills` refreshes and browses user-callable skills from the active plugin catalog. Select a skill and supply its arguments, or type `/skill plugin:name arguments` / `/plugin:name arguments`. Skill invocation follows the existing user-invocation flags and tool restrictions; `@file` attachments also work in skill arguments. If the list is empty, check `lato plugin list` and your plugin skill declarations.

Enter sends; Alt-Enter inserts a newline (Shift-Enter also works when reported by the terminal). Pasting multiline text never submits it. Up/Down moves within the input, then recalls history at the first/last line; returning past the newest item restores the unsent draft. Ctrl-A/Ctrl-E moves to line boundaries, Ctrl-U clears the input, and PgUp/PgDn scrolls the conversation.
